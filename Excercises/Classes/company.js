class Company {
    ...
    addEmployee(user, salary, position, department) {
        if (!user || !position || !department || salary < 0){
    ...
    bestDepartment() {
        ...
        Object.keys(this.departments).forEach((key) => {
            const average = 
                this.departments[key].reduce((acc, current) => acc + current.salary, 0) / 
        ...
        this.departments[sorted[0][0]].sort(
            (a, b) => b.salary - a.salary || a.user.localeCompare(b.user)
            ).forEach((x) => {
